...
ests <- foreach(i = 1:100, .combine = "rbind", .options.RNG = 12102022, # 05102022
                .packages = c("data.table", "survival")) %dorng% {
                  ...
                  model <- em(Surv(time1,time2,status) ~ treat + strata(factor(W)), data = tmp, M = M)
                  ...
                  c(ests1, SE1, ests2, SE2, coef(cox1)[1], coef(cox2)[1], coef(cox3)[1], cox1_cov, cox2_cov, cox3_cov, cox1_se, cox2_se, cox3_se)
