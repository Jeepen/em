...
library(doRNG)
registerDoMC(6)
source("~/Dropbox/phd/Project2/R/estimation.R")
...
## tau <- 1000
lambdaPois <- 2                                           # Parameter til poissonfordeling
D <- 100                                                  # Behandlingslængde
M <- 4

## Simulation
starttime <- Sys.time()
ests <- foreach(i = 1:100, .options.RNG = 27102020, .combine = "rbind") %dorng% {
  ...
  tmp <- do.call("rbind", lapply(1:n, function(i){
    if(treat[i] == 1) data.table(id = i, time1 = 0, time2 = T[i], status = status[i], treat = 1, W = min(obsW[i],M),
                                 statusW = statusW[i], sex = sex[i])
    else data.table(id = i, time1 = c(0, D*W[i]), time2 = c(D*W[i], T[i]), status = c(0, status[i]),
                    treat = c(1,0), W = min(obsW[i],M), statusW = statusW[i], sex = sex[i])
  }))
  ...
  tmp2 <- do.call("rbind", lapply(1:n, function(i){
    ...
    data.table(id = i, time1 = sek, time2 = c(sek[-1], T[i]), status = c(rep(0, length(sek)-1), status[i]),
               sex = sex[i], treat = c(rep(1, obsW[i]), rep(0, length(sek) - obsW[i])), W = c(1:obsW[i], rep(obsW[i], length(sek) - obsW[i])))
  }))
  tmp2$W[tmp2$W >= M] <- M
